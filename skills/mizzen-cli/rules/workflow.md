...
- **核心探索（flat，可拆为多个板块）**：6-10 道深入研究主题的题目。以开放题为主（heavy 追问），按研究目标穿插合适的结构题；不要为了展示题型而使用矩阵、级联、排序或比重题。按维度拆分成 2-3 个板块，每个板块 3-5 道题。
...
质量检查通过后，强烈建议用户先去页面测试访谈效果。展示**预览链接**：

```
{site_url}/interview/{slug}/create/edit#questions
```

对用户说："建议先点击上方预览链接，确认没问题后告诉我，我来发布。如果不需要测试，也可以直接告诉我发布。"
